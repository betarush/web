              ...
              {rates.list.map((rate, index) => (
                <div className="rate" key={rate.key}>
                  <div className="rate-row">
                    <div className="image" style={resizePhoto(rate.product.logo, 100, 100)}>
											<img src={LOGO_URL + '/' + rate.product.logo.name}/>
										</div>

										<div className="desc">
											<div style={{ fontWeight: 'bold' }}>{rate.product.name}</div><br/>
                      <div><strong>You wrote:</strong> {rate.advice}</div><br/>
