	...
	const rateTheCustomer = (id, testInfo, type, productIndex, feedbackIndex) => {
		...
		if (type == "warn" || warningReasonbox.info.type == "warn") {
			if (!warningReasonbox.show) {
				...
				if (reason) {
					...
					rateCustomer(data)
						...
						.then((res) => {
							if (res) {
								if (process.env.REACT_APP_SEGMENT_ON == true) window.analytics.track('ratecustomer', { id: userId, productId: info.id, type: info.type, testingId: info.testInfo.id, testerId: info.testerId, mobile: true });
			...
			rateCustomer(data)
				...
				.then((res) => {
					if (res) {
						if (process.env.REACT_APP_SEGMENT_ON == true) window.analytics.track('ratecustomer', { id: userId, productId: id, type, testingId: testInfo.id, testerId: testInfo.testerId, mobile: true });
